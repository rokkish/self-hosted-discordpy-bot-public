...
def is_counter_group_name_created(name: str) -> bool:
    with session_scope() as session:
        counter_group = session.query(CounterGroup).filter(CounterGroup.name == name).first()
...
def update_counter_group_max_count(name: str, max_count: int) -> None:
    with session_scope() as session:
        ...
        session.add(counter_group)
        session.commit()
        logger.info(f"Updated counter group {counter_group} from {before} to {max_count}")

def get_latest_counter(counter_group_id: int) -> Union[None, Dict[str, Any]]:
    with session_scope() as session:
        counters = session.query(Counter).filter(Counter.counter_group_id == counter_group_id).all()
        if len(counters) == 0:
            return None
...
def record_counter(user_id: int, counter_group_id: int, count: int, attempt: int, status: bool) -> None:
...
def check_increment(user_id: int, counter_group_id: int, count: int, attempt: int) -> bool:
    """Check if the counter should be incremented."""
    with session_scope() as session:
        counters = session.query(Counter).filter(Counter.counter_group_id == counter_group_id, Counter.attempt == attempt).all()
        if len(counters) == 0:
            return (count == 1)
        else:
            ...
            counter_group = session.query(CounterGroup).filter(CounterGroup.id == counter_group_id).first()
...
def try_increment_counter(user_id: int, counter_group_id: int, count: int) -> Dict[str, Any]:
    """Increment the counter for a user."""
